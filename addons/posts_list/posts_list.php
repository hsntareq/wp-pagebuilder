...
class WPPB_Addon_Posts_List {
	...
	public function get_settings() {
		$settings = array(
			'posts_block_title' => array(
				'type'      => 'text',
				'title'     => __('Post Block Title', 'wp-pagebuilder'),
				'std'       => 'Post Heading'
			),
			'posts_block_title_tag' => array(
				'type'      => 'select',
				'title'     => __('Post Block Title Tag', 'wp-pagebuilder'),
				'values'    => array(
					'h2'       => 'H2',
					'h3'       => 'H3',
					'h4'       => 'H4',
	...
	public function render($data = null) {
		...
		$posts_block_title        	= isset($settings['posts_block_title']) ? $settings['posts_block_title'] : 'Post Heading';
		...
		$posts_per_page         = (bool) isset($settings['posts_all']) ?  -1 : ($settings['posts_per_page'] ?? '-1');
		...
		$posts_block_title_tag        = isset($settings['posts_block_title_tag']) ? $settings['posts_block_title_tag'] : 'h2';
...
?>
			...
			<div class="wppb-posts-addon">
				<div class="wppb-posts-addon-content">
					...
					if (!is_array($posts_block_title)) {
						echo "<$posts_block_title_tag class='wppb-post-block-title'>{$posts_block_title} ({$posts_count})</$posts_block_title_tag>";
					}
					?>

					<div class="wppb-addons-list-wrap">
						<?php
						if (!is_array($posts_metadata)) {
							$posts_metadata = explode(',', $posts_metadata);
						}
						...
						?>
							<div class="wppb-addons-list-items">
